...
def monopoly(request, room_name):
    user = request.user
    all_groups_list = []
    ...
    return render(request, 'monopoly/monopoly.html', context={'room_name': room_name, 'active_users' : room.user_set.all(), room_name:room_name})
...
def choose_game(request):
    if request.method == "POST":
        ...
        return redirect(monopoly, room_name)
    ...
    return render(request, 'monopoly/choose_game.html', context={'rooms': groups_list, 'user': request.user})
